...
describe("from-consensus-buff?", () => {
  ...
  const codeBody = `(define-public (test-1)
    (ok (from-consensus-buff? int 0x0000000000000000000000000000000001))
)
(define-public (test-2)
    (ok (from-consensus-buff? uint 0x0000000000000000000000000000000001))
)
(define-public (test-3)
    (ok (from-consensus-buff? { abc: int, def: int } 0x0c00000002036162630000000000000000000000000000000003036465660000000000000000000000000000000004))
)`;
  ...
  describe("in 2.1", () => {
    ...
    test("works for an invalid value", async () => {
      // Build a transaction to call the contract
      let callTxOptions = {
        senderKey: Accounts.WALLET_1.secretKey,
        contractAddress: Accounts.DEPLOYER.stxAddress,
