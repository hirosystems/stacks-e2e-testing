...
describe("buff-to-int-le", () => {
  ...
  const codeBody = `(define-public (test-1)
    (ok (buff-to-int-le 0x01000000000000000000000000000000))
)
(define-public (test-2)
    (ok (buff-to-int-le 0xffffffffffffffffffffffffffffffff))
)
(define-public (test-3)
    (ok (buff-to-int-le 0x))
)`;
  ...
  describe("in 2.1", () => {
    ...
    test("works for a positive int", async () => {
    ...
    test("works for a negative int", async () => {
      // Build a transaction to call the contract
      let callTxOptions = {
        senderKey: Accounts.WALLET_1.secretKey,
        contractAddress: Accounts.DEPLOYER.stxAddress,
